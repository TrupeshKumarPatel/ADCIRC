...
cmake .. -DCMAKE_INSTALL_PREFIX=$adcirc_install -DBUILD_ADCIRC=ON   -DBUILD_PADCIRC=ON   -DBUILD_ADCSWAN=ON   -DBUILD_PADCSWAN=ON   -DBUILD_ADCPREP=ON   -DBUILD_UTILITIES=ON   -DBUILD_ASWIP=ON   -DBUILD_SWAN=ON   -DBUILD_PUNSWAN=ON   -DENABLE_OUTPUT_NETCDF=ON   -DNETCDFHOME=$(nc-config --prefix) -DENABLE_GRIB2=ON -DENABLE_DATETIME=ON
